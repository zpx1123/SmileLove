...
@property (nonatomic, strong ) UIImageView    * dbackview;
@property (nonatomic ,strong)UILabel  * discLable;
@end
...
//        //黑色背景框
//        self.dbackview=[[UIImageView alloc]init];
//        self.dbackview.frame=CGRectMake(0, imageView.frame.size.height-40, self.frame.size.width, 40);
//        NSString* imgPath = [[NSBundle mainBundle] pathForResource:@"icon_home_item_bottom_bg" ofType:@".png"];
//        self.dbackview.image=[UIImage imageNamed:imgPath];
//        [imageView addSubview:self.dbackview];
        
        
        
        //介绍
        UILabel * danceLable = [[UILabel alloc] init];
        danceLable.frame=CGRectMake(0, 0, self.frame.size.width, 40);
        danceLable.font = [UIFont fontWithName:@"Arial" size:14.0f];
        //清空背景颜色
        danceLable.backgroundColor = [UIColor clearColor];
        //设置字体颜色为白色
        danceLable.textColor = [UIColor whiteColor];
        danceLable.font = [UIFont systemFontOfSize:12.0f];
        //文字居中显示
        danceLable.textAlignment = NSTextAlignmentLeft;
        //自动折行设置
        danceLable.lineBreakMode = NSLineBreakByCharWrapping;
        danceLable.numberOfLines = 0;
        [self.dbackview addSubview:danceLable];
        [self.imagePlayerViewDelegate imagePlayerView:self loadImageForImageView:imageView discLable:danceLable index:i];
    }
